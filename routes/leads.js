  ...
  try {
    ...
    for (const row of parsed.data) {
      ...
      try {
        let leadScore = 0;
        try {
          const flaskResponse = await axios.post(`https://bankcp-backend.railway.internal/predict`, row);
...
const createLeadsRouter = (db) => {
  ...
  router.post("/", async (req, res) => {
    ...
    try {
      const flaskResponse = await axios.post(`https://bankcp-backend.railway.internal/predict`, data);
      leadScore = Math.round((flaskResponse.data.probability ?? 0) * 100);
    } catch (err) {
      console.error("❌ Error prediksi model:", err.message);
    }
